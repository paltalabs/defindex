...
use crate::test::defindex_vault::{AssetInvestmentAllocation, AssetStrategySet, ContractError, StrategyAllocation};
use crate::test::{
...
fn deposit_several_assets_get_asset_amounts_per_shares() {
    ...
    result1_should.set(test.token0.address.clone(), 0i128);
    result1_should.set(test.token1.address.clone(), 0i128);
    ...
    // result2_should = {token0: 111, token1: 888}
    let mut result2_should = Map::new(&test.env);
    result2_should.set(test.token0.address.clone(), 111i128);
    result2_should.set(test.token1.address.clone(), 888i128);
    assert_eq!(result2, result2_should);

    // amount should 2 for token0:
    ...
    // result3_should = {token0: 222, token1: 1777}
    let mut result3_should = Map::new(&test.env);
    result3_should.set(test.token0.address.clone(), 222i128);
    result3_should.set(test.token1.address.clone(), 1777i128);
    assert_eq!(result3, result3_should);

    // amount should 3 for token0:
    ...
    // result4_should = {token0: 333, token1: 2666}
    let mut result4_should = Map::new(&test.env);
    result4_should.set(test.token0.address.clone(), 333i128);
    result4_should.set(test.token1.address.clone(), 2666i128);
    assert_eq!(result4, result4_should);

    // amount should 4 for token0:
    ...
    // result5_should = {token0: 444, token1: 3555}
    let mut result5_should = Map::new(&test.env);
    result5_should.set(test.token0.address.clone(), 444i128);
    result5_should.set(test.token1.address.clone(), 3555i128);
    assert_eq!(result5, result5_should);

    // amount should 5 for token0:
    ...
    // result6_should = {token0: 555, token1: 4444}
    let mut result6_should = Map::new(&test.env);
    result6_should.set(test.token0.address.clone(), 555i128);
    result6_should.set(test.token1.address.clone(), 4444i128);
...
fn deposit_and_invest_several_assets_get_asset_amounts_per_shares() {
    ...
    let token_0_invested_funds = defindex_contract.fetch_current_invested_funds().get(test.token0.address.clone()).unwrap();
    let token_1_invested_funds = defindex_contract.fetch_current_invested_funds().get(test.token1.address.clone()).unwrap();
    let token_0_idle_funds = defindex_contract.fetch_current_idle_funds().get(test.token0.address.clone()).unwrap();
    let token_1_idle_funds = defindex_contract.fetch_current_idle_funds().get(test.token1.address.clone()).unwrap();
