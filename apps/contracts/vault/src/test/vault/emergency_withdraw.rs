...
use crate::test::{
    create_defindex_vault, create_strategy_params_token_0, create_strategy_params_token_1, defindex_vault::{AssetInvestmentAllocation, AssetStrategySet, ContractError, RolesDataKey, StrategyAllocation}, DeFindexVaultTest
};
extern crate std;
...
fn error() {
    ...
    let result = defindex_contract.try_emergency_withdraw(
        &strategy_params_token_0.first().unwrap().address,
        &test.emergency_manager,
    );
    ...
    defindex_contract.unpause_strategy(&strategy_params_token_0.first().unwrap().address, &test.manager);
    ...
    //Wrong strategy
    let result = defindex_contract.try_emergency_withdraw(
        &strategy_params_token_1.first().unwrap().address,
        &test.emergency_manager,
    );
    assert_eq!(result, Err(Ok(ContractError::StrategyNotFound)));
    ...
    //paused strategy
    defindex_contract.pause_strategy(&strategy_params_token_0.first().unwrap().address, &test.manager);
    //First emergency withdraw
    let result = defindex_contract.try_emergency_withdraw(
        &strategy_params_token_0.first().unwrap().address,
        &test.emergency_manager,
