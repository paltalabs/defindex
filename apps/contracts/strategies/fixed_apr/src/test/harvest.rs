use soroban_sdk::{testutils::Ledger, token::StellarAssetClient, vec, IntoVal, Val, Vec};
...
fn test_harvest_yields_multiple_users() {
    ...
    assert_eq!(test.token.balance(&test.strategy.address), starting_amount);
    ...
    test.strategy.harvest(&users[1]);
    test.strategy.harvest(&users[2]);
    test.strategy.harvest(&users[3]);
    ...
    assert_eq!(test.strategy.balance(&users[0]), user1_amount + user1_expected_reward);
    assert_eq!(test.strategy.balance(&users[1]), user2_amount + user2_expected_reward);
    assert_eq!(test.strategy.balance(&users[2]), user3_amount + user3_expected_reward);
    assert_eq!(test.strategy.balance(&users[3]), user4_amount + user4_expected_reward);
}